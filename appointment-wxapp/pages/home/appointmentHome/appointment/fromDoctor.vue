	...
	<view>
		...
		<view class="record-box">
			<view class="doctor-box" v-for="(item,index) in doctorRecordList" :key='item.id'>
				<text class="doctor" :data-name='item.id' @click="toDoctorBrief(item)"
				:class="isActived == item.id?'doctor-blue-background':''">{{item.name}}</text>
			</view>
		</view>
		...
		<view class="doctor-outbox" style="margin-top: 60rpx;"
		 @click="toDoctorBrief(item)" v-for="(item, index) in doctorList" :key='item.id'>
			...
			<view class="doctor-info">
				...
				<text class="gray-text cut-long-text">简介：{{item.id}}</text>
			</view>
		</view>
